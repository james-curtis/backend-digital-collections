...
class Config extends Backend
{
    ...
    public function edit($ids = null)
    {
        if ($this->request->isPost()) {
            ...
            $config=Db::name('config')->where('group','wenchangchain')->field('id,title,value')->select();
            
             if(array_key_exists('status', $row)){
                if($config[3]['value']!=$row['status']){
                    $configs=array('apiKey'=>$row['apiKey'],'apiSecret'=>$row['apiSecret'],'url'=>$row['url']);
                $account=CreateChainAccount($configs,$row['names']);
                if(array_key_exists('error', $account)){
                    $this->error(__($account['error']['code']));
                }else{
                   $row['account']=$account['data']['account']; 
                }
