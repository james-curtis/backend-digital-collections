...
class Bill extends Backend
{
    ...
    public function index()
    {
        ...
        if ($this->request->isAjax()) {
            ...
            foreach ($list as $row) {
                $row->visible(['id', 'account', 'before_account', 'after_account', 'bill_type', 'remark', 'type', 'create_time']);
                $row->visible(['currency']);
                $row->getRelation('currency')->visible(['name']);
                $row->visible(['users']);
                $row->getRelation('users')->visible(['member', 'phone']);
    ...
    public function export()
    {
        if ($this->request->isPost()) {
            ...
            $whereIds = $ids == 'all' ? '1=1' : ['bill.id' => ['in', explode(',', $ids)]];
            ...
            $columns = "bill.id,users.phone,currency.name,bill.account,bill.bill_type,bill.remark,bill.type,bill.create_time";
//            print_r($worksheet);
//            exit();
//            print_r($where);
//            exit();
